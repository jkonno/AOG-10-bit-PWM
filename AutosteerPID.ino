void calcSteeringPID(void) 
 {  
  ...
  {
    newMax = (errorAbs * 4*highLowPerDeg) + 4*steerSettings.lowPWM;
  }
  else newMax = 4*steerSettings.highPWM;
    
  //add min throttle factor so no delay from motor resistance.
  if (pwmDrive < 0 ) pwmDrive -= 4*steerSettings.minPWM;
  else if (pwmDrive > 0 ) pwmDrive += 4*steerSettings.minPWM;
